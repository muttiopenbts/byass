...
Purpose: Passively look for potential urls hidden within client side content,
i.e. js, html, css

TODO:
- Big cleanup, more use case testing.
- Checking hostname + path + js-filename, as unique and checking once, but query
js might have different content based on query param value. e.g. ?ver=1.2.
Might be better to just recheck or quicker to calculate a hash and rescan if
hash change.
...
class BurpExtender(IBurpExtender, ITab, IHttpListener, IMessageEditorController, AbstractTableModel):
    ...
    def processHttpMessage(self, toolFlag, messageIsRequest, messageInfo):
        ...
        if messageIsRequest:
            ...
            # Don't process already processed responses
            if self.hosts_dic.get(hostname+path):
                return
            ...
            # Does request contain item of interest, javascript files.
            if (file_extension.lower() == 'js'):
                comment += 'JsUrlResp '
                for url in self.getRegexedUrls(body):
                    if self.hosts_dic.get(hostname+path):
                        if not url in self.hosts_dic.get(hostname+path):
                            # Append url if not known
                            self.hosts_dic.get(hostname+path).append(url)
                    else:
                        # New Entry
                        self.hosts_dic.setdefault(hostname+path,[]).append(url)

            elif (mime_type.lower() == 'json'):
                # Does request contain item of interest, javascript files.
                comment += 'JsonUrlResp '
                for url in self.getRegexedUrls(body):
                    if self.hosts_dic.get(hostname+path):
